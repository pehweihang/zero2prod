...
async fn spawn_app() -> TestApp {
    ...
    let server = run(listener, connection_pool.clone()).expect("Faild to bind address");
...
async fn configure_database(database_settings: &DatabaseSettings) -> PgPool {
    ...
    let connection_pool = PgPool::connect_with(database_settings.with_db())
        .await
        .expect("Failed to connect to Postgres");
...
async fn subscribe_returns_200_for_valid_form_data() {
    ...
    let response = client
        ...
        .send()
        .await
        .expect("Failed te execute request");
...
async fn subscribe_returns_400_when_data_is_missing() {
    ...
    for (invalid_body, error_message) in test_cases {
        let response = client
            .post(format!("{}/subscriptions", test_app.address))
            .header("Client-Type", "x-www-form-urlencoded")
            .body(invalid_body)
            .send()
            .await
            .expect("Failed to execute request");
...
#[tokio::test]
async fn subscribe_returns_400_whet_fields_are_present_but_invalid() {
    let test_app = spawn_app().await;
    let client = reqwest::Client::new();
    ...
    let test_cases = vec![
        ...
        ("name=Ursula&email=notemail", "invalid email"),
    ];
    ...
    for (invalid_body, error_message) in test_cases {
        ...
        assert_eq!(
            400,
            response.status().as_u16(),
            "API did not fail with 400 with {}",
            error_message
        );
    }
}
